...
import { MP4Patcher } from "./mp4-patcher.js";
...
export class FFmpegRenderer {
  ...
  private patcher: MP4Patcher = new MP4Patcher();
      ...
      if (options.quality === "quality" && encoder === "libx264") {
        await this.run(
          [
            ...
            String(crf),
            "-maxrate",
            `${plan.output.maxVideoBitrateKbps}k`,
            "-bufsize",
            `${plan.output.bufferSizeKbps}k`,
            "-progress",
            "pipe:1",
            "-nostats",
            tempOutputPath,
        ...
        const rateControl = [
          ...
          String(crf),
          "-b:v",
          `${plan.output.videoBitrateKbps}k`,
          "-maxrate",
          `${plan.output.maxVideoBitrateKbps}k`,
          "-bufsize",
          `${plan.output.bufferSizeKbps}k`,
  ...
  private cleanupPartialFile(): void {
    if (this.currentTempPath && existsSync(this.currentTempPath)) {
      try {
        unlinkSync(this.currentTempPath);
      } catch {
        // Игнор
      }
