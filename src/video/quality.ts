...
export async function calculateMetrics(sourcePath: string, outputPath: string): Promise<QualityMetricsResult> {
  ...
  const sourceRotation = getRotationFilter(source.rotation);

  const filter = [
    `[0:v]${sourceRotation},scale=${display.width}:${display.height}:force_original_aspect_ratio=disable:flags=lanczos,setsar=1,fps=${fps},settb=1/${fps},setpts=PTS-STARTPTS[ref]`,
