...
function resolveLocalTelegramFile(filePath: string): string | null {
  ...
  try {
    if (fs.existsSync(directCandidate) && fs.statSync(directCandidate).isFile()) return path.resolve(directCandidate);
  } catch {}
...
async function copyTelegramFileFromDocker(filePath: string, destPath: string): Promise<void> {
  ...
  if (!fs.existsSync(destPath)) throw new Error(`Docker copied no file to: ${destPath}`);
...
async function downloadLocalTelegramFile(filePath: string, destPath: string): Promise<void> {
  ...
  if (!fs.existsSync(destPath)) throw new Error(`Telegram file was not copied: ${filePath}`);
  const size = fs.statSync(destPath).size;
...
function getMainMenu(lang: "kk" | "ru") {
  ...
  return new Keyboard().text(m.video4k).row().text(m.profile).text(m.myLimit).row().text(m.referral).text(m.buyTariff).row().text(m.rules).resized();
}
